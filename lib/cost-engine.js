...
class CostEngine {
  ...
  getModelTier(modelName) {
    const tiers = {
      fast: { models: ['haiku', 'haiku-4.5', 'claude-haiku', 'sonnet-4.5', 'claude-3.5-haiku'], default: 'haiku' },
      standard: { models: ['sonnet', 'sonnet-5', 'claude-sonnet', 'sonnet-4.6', 'claude-3.5-sonnet', 'claude-3-sonnet'], default: 'sonnet' },
      premium: { models: ['opus', 'opus-5', 'claude-opus', 'opus-4.8', 'claude-3-opus', 'fable', 'fable-5', 'claude-3.5-fable'], default: 'opus' }
    };
  ...
  resolveModelName(alias) {
    const modelMap = {
      ...
      'haiku-4.5': 'haiku', 'sonnet-5': 'sonnet', 'sonnet-4.6': 'sonnet',
      'opus-5': 'opus', 'opus-4.8': 'opus',
      'claude-haiku': 'haiku', 'claude-sonnet': 'sonnet', 'claude-opus': 'opus',
      'claude-3.5-haiku': 'haiku', 'claude-3.5-sonnet': 'sonnet', 'claude-3.5-fable': 'opus',
      fable: 'opus', 'fable-5': 'opus', 'claude-3-opus': 'opus',
      'claude-3-sonnet': 'sonnet', 'claude-3-haiku': 'haiku'
    };
  ...
  getAutoModelForComplexity(complexity, taskType) {
    ...
    const preferredModel = config.preferred_model || 'opus';
    const ceilingTier = this.getModelTier(preferredModel);

    let targetTier;
    if (complexity < 0.3 || taskType === 'simple') {
      targetTier = 'fast';
    } else if (complexity > 0.7 || taskType === 'complex') {
      targetTier = ceilingTier;
    } else {
      targetTier = 'standard';
    }

    if (targetTier === 'fast' && ceilingTier === 'fast') targetTier = 'fast';
    if (targetTier === 'standard' && ceilingTier === 'fast') targetTier = 'fast';
    if (targetTier === 'standard' && ceilingTier === 'standard') targetTier = 'standard';
    if (targetTier === 'premium' && ceilingTier === 'fast') targetTier = 'fast';
    if (targetTier === 'premium' && ceilingTier === 'standard') targetTier = 'standard';

    const tierDefaults = { fast: 'haiku', standard: 'sonnet', premium: 'opus' };
    const tierPreferred = { fast: config.preferred_haiku, standard: config.preferred_sonnet, premium: config.preferred_opus };

    const model = tierPreferred[targetTier] || tierDefaults[targetTier];
    return this.resolveModelName(model);
  }

  getAutoModelDecision(context) {
    const complexity = this.assessComplexity(context);
    const taskType = this.classifyTask(context);
    const model = this.getAutoModelForComplexity(complexity, taskType);
    const routingSavings = this.calculateSavings(model, complexity);

    const decision = {
      model,
      reason: `Complexity ${complexity.toFixed(2)}, task: ${taskType}`,
      complexity,
      taskType,
      savings: routingSavings,
      autonomous: true,
      ceilingModel: this.configManager.getCostConfig().preferred_model || 'opus',
      ceilingTier: this.getModelTier(this.configManager.getCostConfig().preferred_model || 'opus'),
      selectedTier: this.getModelTier(model),
      timestamp: new Date().toISOString()
    };

    if (!this.autoRoutingLog) this.autoRoutingLog = [];
    this.autoRoutingLog.push(decision);
    this.costMetrics.modelRoutingDecisions++;
    this.recordSavings('model_routing', routingSavings.tokens);
  ...
  getSupportedAgents() {
    const agents = ['claude', 'cursor', 'codex', 'gemini', 'windsurf', 'copilot', 'cline', 'opencode', 'hermes', 'kilo'];
    const configPaths = { claude: '.claude', cursor: '.cursor', codex: '.codex', gemini: '.gemini', windsurf: '.windsurf', copilot: '.github', cline: '.cline', opencode: '.opencode', hermes: '.hermes', kilo: '.kilo' };
    return agents.map(agent => ({ agent, configDir: configPaths[agent] || `.${agent}`, supported: true }));
  }
}
